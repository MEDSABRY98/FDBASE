...
async function loadAfconEgyptTeamsData(forceRefresh = false) {
    try {
        showLoading();
...
async function loadTrophySeasons() {
    try {
        const response = await fetch('/api/afcon-egypt-teams/trophy-seasons');
        if (!response.ok) {
...
async function refreshAfconEgyptTeamsData() {
    ...
    try {
        await loadAfconEgyptTeamsData(true);
