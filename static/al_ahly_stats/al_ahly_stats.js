...
function calculateOverviewStats() {
    ...
    const cleanSheetsAgainst = matches.filter(m => m.goalsFor === 0).length;
    
    // Calculate draws with goals vs no goals from raw records
    const filteredRecords = alAhlyStatsData.filteredRecords || alAhlyStatsData.allRecords || [];
...
function showPlayerSubTab(arg1, arg2) {
    ...
    if (selectedPlayer) {
        loadPlayerSubTabData(subTabName);
    }
}

// Load player sub-tab data based on current selection (Excel-only)
function loadPlayerSubTabData(subTabName) {
    const selectedPlayer = document.getElementById('player-search') ? document.getElementById('player-search').value.trim() : '';
    const teamFilter = document.getElementById('player-team-filter') ? document.getElementById('player-team-filter').value : '';
    if (!selectedPlayer) return;
    switch (subTabName) {
        case 'overview':
            if (typeof loadPlayerOverviewWithFilter === 'function') loadPlayerOverviewWithFilter(teamFilter);
            break;
        case 'matches':
            loadPlayerMatchesWithFilter(teamFilter);
            break;
        case 'championships':
            loadPlayerChampionshipsStats();
            break;
...
function loadPlayerAssistDetails() {
    try {
        ...
        const filteredDetails = playerDetails.filter(r => {
            if (teamLower) {
                const team = normalizeStr(r.TEAM || r['AHLY TEAM'] || '').toLowerCase();
                return team === teamLower;
            }
            return true;
        });
...
function displayMatchLineup(matchId) {
    ...
    matchLineup.forEach((player, index) => {
        ...
        if (index < 11) {
            ...
            if (playerOut && minOut) {
                status = `Substitute (in for ${playerOut} at ${minOut}')`;
            } else if (playerOut) {
                status = `Substitute (in for ${playerOut})`;
            } else if (minOut) {
                status = `Substitute (at ${minOut}')`;
            } else {
                status = 'Substitute';
            }
        }
        ...
        html += `
            <tr>
                ...
                <td>${minutes}'</td>
                <td style="color: ${index < 11 ? '#059669' : '#f59e0b'};">${status}</td>
            </tr>
        `;
    });
...
function displayMatchGoals(matchId) {
    ...
    const ahlyGoals = matchGoals.filter(g => {
        ...
        return team.includes('ahly') || team.includes('الأهلي');
    });
